...
$head=$head."<div style='font-size:14px;border-bottom:1px solid #000;font-weight:800;width:40%;position:absolute;top:170px;left:70px;padding:0 0 0 10px;'>"." 予定工期 : ".date('Y/m/d',strtotime($main->jutyu_D))." から ".date('Y/m/d',strtotime($main->nouki))."</div>";
$head=$head."<div style='font-size:14px;border-bottom:1px solid #000;font-weight:800;width:30%;position:absolute;top:195px;left:70px;padding:0 0 0 10px;'>"."完了予定日: ".date('Y/m/d',strtotime($main->nouki))."</div>";
$head=$head."<div style='font-size:18px;border-bottom:1px solid #000;font-weight:800;width:282px;position:absolute;top:230px;left:70px;padding:0 0 0 10px;'>"." 合計金額 : ￥".number_format($kei->total)."<span style='font-size:12px'>(税抜)</span></div>";
$yb=$jisya->j_zip;
...
<p style='margin:0px 20px 10px 0px;font-size:16px;font-weight:0;'>".$jisya->j_name."</p>
<p style='font-weight:0;margin:0px;'>〒".$yb." ".$jisya->j_add."</p>
<p style='margin:0px;font-weight:0;'>TEL:".$tel."/FAX:".$tel."</p>
</div></div>";
$ts="<div id='pdf'><table class='pm' style='margin-top:5px;font-size:15px;'><tr><th style='width:30px;'>#</th><th colspan='4'>図番</th><th colspan='4'>品名</th><th style='width:60px;!important'>数量</th><th>単位</th><th style='width:80px;!important'>単価</th><th style='width:80px;!important'>金額</th></tr>";
$st='';
$l=1;
...
for($k=0;$k<$pages;$k++){
	$page=$k+1;
		...
		foreach($qarr[$k]->result() as $v){
			//$st=$st.'<tr><td style="padding:0.25em;">'.$stLen.'</td><td colspan="4" style="text-align:left;">'.$v->zuban.'</td><td colspan="4" style="text-align:left;padding-left:3px;">'.$v->hinmei.'</td><td>'.number_format($v->quan).'</td><td>'.number_format($v->tanka).'</td><td>'.number_format($v->z_total).'</td></tr>';

			/*$stLen=strlen($v->zuban)+strlen($v->hinmei);
			if ($stLen>56){
				$st=$st.'<td colspan="4" style="text-align:left;padding-left:3px;">'.$v->hinmei.'</td>'
			}*/

			$st=$st.'<tr><td style="padding:0.25em;">'.$v->num.'</td><td colspan="4" style="text-align:left;">'.$v->zuban.'</td><td colspan="4" style="text-align:left;padding-left:3px;">'.$v->hinmei.'</td><td>'.number_format($v->quan).'</td><td style="text-align:center;">'.$v->tani.'</td><td>'.number_format($v->tanka).'</td><td>'.number_format($v->z_total).'</td></tr>';
			//$l++;
		}
