<?php
	...
	$tb='tmp_zai'.$sid;
		...
		$this->db->select('z_store')->distinct('z_store')->from($tb);
		$query=$this->db->get();
...
$header="<p style='width:100%;text-align:center;font-size:24px;font-weight:0;'>材料発注書</p>";
...
$me="<span><div style='text-align:right;font-weight:0;'><p style='margin:30px 0 23px 0;font-weight:0;'>".$jisya->j_name."</p><p style='font-weight:0;margin:0px;'>〒".$jisya->j_zip." ".$jisya->j_add."</p>
...
$gk=0;
	for($i=0;$i<$cnt;$i++){
		foreach($zarr[$i]->result() as $g){//発注先ごと
			...
			$head=$head."<div style='font-size:15px;border-bottom:1px solid #000;font-weight:800;width:40%;position:absolute;top:100px;padding:0 0 0 10px;'>"." 発 注 日 : ".$_GET['date']."</div>";
			if($_GET['ck']==0){
				$head=$head."<div style='font-size:15px;font-weight:800;position:absolute;top:100px;left:300px;padding:0 0 0 10px;'>"." 希望納期 : 通常</div>";
			}else{
				$head=$head."<div style='font-size:15px;font-weight:800;position:absolute;top:100px;left:300px;padding:0 0 0 10px;'>"." 希望納期 : 特急</div>";
			}
			...
			$head=$head."<div style='font-size:15px;border-bottom:1px solid #000;font-weight:800;width:40%;position:absolute;top:130px;padding:0 0 0 10px;'>"." 納品場所 : ".$_GET['place']."</div>";
