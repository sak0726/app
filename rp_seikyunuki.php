...
$header="<p style='width:30%;text-align:center;margin:10px auto;font-size:24px;font-weight:0;'>御請求書</p>";
...
$head="<div style='font-size:14px;font-weight:0;position:absolute;top:15px;right:15px;'>発行日 ".date('Y/m/d')."</div>";
...
$head=$head."<div style='font-size:18px;border-bottom:1px solid #000;font-weight:800;width:282px;position:absolute;top:230px;left:70px;padding:0 0 0 10px;'>"." 御請求額 : ￥".number_format($kei->total+($kei->total*0.1))."</div>";
$head=$head."<div style='font-size:12px;border-bottom:1px solid #000;font-weight:800;width:33%;position:absolute;top:260px;left:90px;padding:0 0 0 10px;'>"." 10%対象額 : ￥".number_format($kei->total)." <span> 消費税額 : ￥".number_format($kei->total*0.1)."</span></div>";
$head=$head."<div style='font-size:12px;border-bottom:1px solid #000;font-weight:800;width:33%;position:absolute;top:280px;left:90px;padding:0 0 0 10px;'>"." 8% 対象額 :<span style='color:#fff;'> ￥".number_format($kei->total)." <span style='color:#000;'>消費税額 :</span> ￥".number_format($kei->total*0.1)."</span></div>";
...
<p style='margin:0px 0px 0px 0;font-size:14px;font-weight:0;'>登録番号 : ".$jisya->j_tourokuNum."</p>
...
<p style='margin:0px;font-weight:0;font-size:12px;'>口座:<span style='font-size:12px'>".$jisya->j_kouza."</span></p>
</div></div>";
$ts="<div id='pdf'><table class='pm' style='margin-top:5px;font-size:15px;'><tr><th style='width:30px;'>#</th><th colspan='7'>項目</th><th style='width:60px;!important'>数量</th><th style='width:60px;!important'>単位</th><th style='width:80px;!important'>単価</th><th style='width:80px;!important'>金額</th></tr>";
$st='';
$l=1;
...
for($k=0;$k<$pages;$k++){
	...
	foreach($qarr[$k]->result() as $v){
		...
		$tani;
		$tanka;
		$total=number_format($v->z_total);
		if($v->quan===NULL || $v->quan=="0"){
			$quan="";
			$tani="";
		}else{
			$quan=number_format($v->quan);
			$tani=$v->tani;
		}
		$quan=number_format($v->quan);
		$tani=$v->tani;
		if($v->tanka===NULL){
			...
			$tanka=number_format($v->tanka);
		}
		if($total=="0"){
			$st=$st.'<tr><td style="padding:0.25em;">'.$v->num.'</td>
			<td colspan="11" style="text-align:center;padding-left:5px;background-color:#efefef;">'.$v->zuban.'  '.$v->hinmei.'</td></tr>';
		}else{
			$st=$st.'<tr><td style="padding:0.25em;">'.$v->num.'</td>
			<td colspan="7" style="text-align:left;padding-left:5px;">'.$v->zuban.'  '.$v->hinmei.'</td><td>'.$quan.'</td><td style="text-align:center;">'.$tani.'</td><td>'.$tanka.'</td><td>'.$total.'</td></tr>';
		}
	}
